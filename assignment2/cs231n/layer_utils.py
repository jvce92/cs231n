...
def affine_relu_bn_forward(x, w, b, gamma, beta, bn_param):
    """
    Convenience layer that performs an affine transform followed by a ReLU and a batch norm
    ...
    out, bn_cache = batchnorm_forward(b, gamma, beta, bn_param)
    cache = (fc_cache, relu_cache, bn_cache)
    return out, cache

def affine_relu_bn_backward(dout, cache):
    """
    Backward pass for the affine-relu-bn convenience layer
    """
