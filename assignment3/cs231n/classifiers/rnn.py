...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == "rnn":
            ...
            raise ValueError("Input a valida cell type (rnn or lstm)")
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(1, max_length):
            captions_in = captions[:, t-1]
            embed, _ = word_embedding_forward(captions_in[:, np.newaxis], W_embed)
            ...
            best_scores_idx = np.squeeze(np.argmax(scores, axis=2))
            captions[:, t] = best_scores_idx
            prev_h = h
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
